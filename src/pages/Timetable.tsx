...
import {
  ...
  LayoutList, LayoutGrid, CalendarClock,
} from "lucide-react";
...
const REPEAT_LABELS: Record<RepeatType, string> = { once: "1️⃣ Once", weekly: "🔁 Weekly", daily: "🔁 Daily" };
...
export default function Timetable() {
  ...
  const handleSave = (isEdit: boolean) => {
    ...
    if (isEdit && editSession) {
      ...
      if (formRepeat === "daily") {
        DAYS.forEach((d) => {
          next[d] = [...(next[d] || []), { ...newSession, id: d === addDay ? newSession.id : genId() }];
        });
  ...
  const handleMove = (sessionId: string, fromDay: string, targetDay: string) => {
    const daySessions = schedule[fromDay] || [];
  ...
  const handleComplete = (session: Session, day: string) => {
    if (strictMode) {
      ...
      toggleComplete(session.id, day);
    }
  };

  const toggleComplete = (id: string, day: string) => {
    const next = { ...schedule };
  ...
  const submitSummary = () => {
    ...
    toggleComplete(summarySession.id, summaryDay);
  ...
  const renderSessionCard = (session: Session, index: number, day: string, compact = false) => {
    ...
    return (
      <div
        ...
        className={`glass-card ${compact ? "p-2.5" : "p-4"} flex items-center justify-between border-l-4 ${
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
              {compact && (
                <span className="text-[10px] font-mono text-muted-foreground">{session.time}</span>
              )}
              {session.topic && (
                <p className="text-[10px] text-muted-foreground truncate max-w-[120px]">{session.topic}</p>
              )}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {session.completed ? (
            <button
              onClick={() => toggleComplete(session.id, day)}
            ...
            <button
              onClick={() => handleComplete(session, day)}
              ...
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent>
                  {DAYS.filter((d) => d !== day).map((d) => (
                    <DropdownMenuItem key={d} onClick={() => handleMove(session.id, day, d)}>
                      {d}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuSubContent>
  ...
  return (
    <AppLayout>
              ...
              return (
                <div key={day} className="min-w-[160px]">
                  <div className={`text-center text-xs font-semibold py-2 rounded-t-lg ${
                    day === selectedDay ? "gradient-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"
                  }`}>
                    {day.slice(0, 3)}
                    <span className="ml-1 opacity-70">({daySessions.length})</span>
                  </div>
                  <div className="border border-t-0 border-border rounded-b-lg p-1.5 space-y-1.5 min-h-[120px] bg-card/50">
                    {daySessions.length === 0 ? (
                      <button
                        onClick={() => openAdd(day)}
                        className="w-full py-6 text-center text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted/50 rounded-lg transition-colors"
                      >
                      ...
                      <>
                        ...
                        <button
                          onClick={() => openAdd(day)}
                          className="w-full py-1 text-center text-[10px] text-muted-foreground/50 hover:text-muted-foreground border border-dashed border-border rounded-lg transition-colors"
                        >
            ...
            <div className="space-y-3">
              {sortedSessions.length === 0 ? (
                ...
                >
                  ...
                  <Button onClick={() => openAdd(selectedDay)} size="lg">
                    <Plus className="w-4 h-4 mr-1" /> Add First Session
                  </Button>
                </motion.div>
              ) : (
                <>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Session — {addDay}</DialogTitle>
            <DialogDescription>Create a new study session.</DialogDescription>
          </DialogHeader>
          {sessionFormContent}
          <DialogFooter>
      ...
      <Dialog open={!!summarySession} onOpenChange={(v) => !v && setSummarySession(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Write at least <strong>3 sentences</strong> about what you learned.</DialogDescription>
          </DialogHeader>
